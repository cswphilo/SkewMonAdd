...
-- ∨l-ri-∧r* : {Γ : Cxt} {A' B' A B : Fma}
--   → {Φ Ψ : List Fma}
--   → (fs : List (Σ Pos (_∣_⊢li_ (just A') Γ)))
--   → (fs' : List (Σ Pos (_∣_⊢li_ (just B') Γ)))
--   → (SF1 : SubFmas Φ A) (SF2 : SubFmas Ψ B)
--   → 
...
... | (C₁ , f₁) ∷ fs , .(mapList (λ x₂ → proj₁ (proj₁ x₂)) ((C₁ , f₁) ∷ fs)) , refl , SF1 , refl | (C₂ , f₂) ∷ fs' , .(mapList (λ x₂ → proj₁ (proj₁ x₂)) ((C₂ , f₂) ∷ fs')) , refl , SF2 , refl = {!   !}
