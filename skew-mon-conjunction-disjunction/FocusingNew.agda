...
isOK2isOKC₁ : {t : Tag} {l : List Tag} {Γ : Cxt} {A B : Fma} {C : Pos}
  ...
  → (ok : isOK (t ∷ l))
  → isOKC₁ (t ∷ l)
isOK2isOKC₁  f (isR _) = stopR _
isOK2isOKC₁  f (isC₁ l x) = cont l
isOK2isOKC₁  f (isC₂ _ x) = stopC₂ _
...
... | inj₂ ((t , .C' , f'') ∷ fs' , refl , ok) = inj₂ ((P , C , passT f) ∷ (t , C' , f'') ∷ fs'  , refl , isP _ ok)
check-focus (.(` X , tt) , f2li (ax {X})) [] = inj₂ (((R , ((` X , tt) , ax)) ∷ []) , refl , {!   !})
check-focus (.(I , tt) , f2li Ir) [] = inj₂ (((R , (I , tt) , Ir) ∷ []) , (refl , {!   !}))
check-focus {S , q} (.(A ⊗ B , tt) , f2li (⊗r l {S = .(irr (S , q)) , p} {A = A} {B} ok eq f g)) [] 
  rewrite irr-eq S p | isIrr-unique S p q = inj₂ (((R , ((A ⊗ B , tt) , (⊗rT l ok eq f g))) ∷ []) , refl , {!   !})
...
check-focus {S , q} (.(A ∨ B , tt) , f2li (∨r₁ l {S = .(irr (S , q)) , p} {A = A} {B} ok f)) [] 
  rewrite irr-eq S p | isIrr-unique S p q = inj₂ ((R , (A ∨ B , tt) , (∨r₁T l ok f)) ∷ [] , refl , {!   !})
check-focus {S , q} (.(A ∨ B , tt) , f2li (∨r₂ l {S = .(irr (S , q)) , p} {A = A} {B} ok f)) [] 
  rewrite irr-eq S p | isIrr-unique S p q = inj₂ ((R , (A ∨ B , tt) , (∨r₂T l ok f)) ∷ [] , refl , {!   !})
...
check-focus {.(just (A ∧ B)) , snd} (.(A' ⊗ B' , tt) , f2li (⊗r l {A = A'} {B'} ok eq f g)) ((C' , .(f2li (∧l₁ f''))) ∷ .(mapList (λ x → proj₁ x , f2li (∧l₁ (proj₂ x))) fs')) | inj₁ (inj₁ (A , B , (.C' , f'') ∷ fs' , refl , refl)) = 
  inj₂ ((R , ((A' ⊗ B') , tt) , ⊗rT l ok eq f g) ∷ (C₁ , C' , (∧l₁T f'')) ∷ (mapList (λ x → C₁ , proj₁ x , ∧l₁T (proj₂ x)) fs') , refl , {!   !})
...
check-focus {.(just (A ∧ B)) , snd} (C , f2li (∧l₂ f)) ((C' , .(f2li (∧l₁ f''))) ∷ .(mapList (λ x → proj₁ x , f2li (∧l₁ (proj₂ x))) fs')) | inj₁ (inj₁ (A , B , (.C' , f'') ∷ fs' , refl , refl)) = 
  inj₂ ((C₂ , C , ∧l₂T f) ∷ (C₁ , C' , ∧l₁T f'') ∷ (mapList (λ x → C₁ , proj₁ x , ∧l₁T (proj₂ x)) fs') , refl , {!   !})
check-focus {.(just (A ∧ B)) , snd} (.(A' ∨ B' , tt) , f2li (∨r₁ l {A = A'} {B'} ok f)) ((C' , .(f2li (∧l₁ f''))) ∷ .(mapList (λ x → proj₁ x , f2li (∧l₁ (proj₂ x))) fs')) | inj₁ (inj₁ (A , B , (.C' , f'') ∷ fs' , refl , refl)) = 
  inj₂ ((R , (A' ∨ B' , tt) , ∨r₁T l ok f) ∷ (C₁ , C' , ∧l₁T f'') ∷ (mapList (λ x → C₁ , proj₁ x , ∧l₁T (proj₂ x)) fs') , refl  , {!   !})
check-focus {.(just (A ∧ B)) , snd} (.(A' ∨ B' , tt) , f2li (∨r₂ l {A = A'} {B'} ok f)) ((C' , .(f2li (∧l₁ f''))) ∷ .(mapList (λ x → proj₁ x , f2li (∧l₁ (proj₂ x))) fs')) | inj₁ (inj₁ (A , B , (.C' , f'') ∷ fs' , refl , refl)) =
  inj₂ ((R , (A' ∨ B' , tt) , ∨r₂T l ok f) ∷ (C₁ , C' , ∧l₁T f'') ∷ (mapList (λ x → C₁ , proj₁ x , ∧l₁T (proj₂ x)) fs') , refl  , {!   !})
...
check-focus {.- , snd} ((_ , .tt) , f2li (⊗r l ok eq f g)) ((C' , .(f2li (pass f'))) ∷ .(mapList (λ x → proj₁ x , f2li (pass (proj₂ x))) fs')) | inj₁ (inj₂ (inj₂ (A , Γ , (.C' , f') ∷ fs' , refl , refl , refl))) = inj₂ ((R , _ , ⊗rT l ok eq f g) ∷ (P , C' , passT f') ∷ mapList (λ x → P , proj₁ x , passT (proj₂ x)) fs' , refl , {!   !})
check-focus {.- , snd} ((_ , .tt) , f2li (∨r₁ l ok f)) ((C' , .(f2li (pass f'))) ∷ .(mapList (λ x → proj₁ x , f2li (pass (proj₂ x))) fs')) | inj₁ (inj₂ (inj₂ (A , Γ , (.C' , f') ∷ fs' , refl , refl , refl))) = inj₂ ((R , _ , ∨r₁T l ok f) ∷ (P , C' , passT f') ∷ mapList (λ x → P , proj₁ x , passT (proj₂ x)) fs' , refl , {!   !})
check-focus {.- , snd} ((_ , .tt) , f2li (∨r₂ l ok f)) ((C' , .(f2li (pass f'))) ∷ .(mapList (λ x → proj₁ x , f2li (pass (proj₂ x))) fs')) | inj₁ (inj₂ (inj₂ (A , Γ , (.C' , f') ∷ fs' , refl , refl , refl))) = inj₂ ((R , _ , ∨r₂T l ok f) ∷ (P , C' , passT f') ∷ mapList (λ x → P , proj₁ x , passT (proj₂ x)) fs' , refl , {!   !})
check-focus {.(just (` X)) , snd} (.(` X , tt) , f2li (ax {X})) ((C' , .(f2li (untagF f'))) ∷ .(mapList (λ x → proj₁ (proj₂ x) , f2li (untagF (proj₂ (proj₂ x)))) fs')) | inj₂ ((t , C' , f') ∷ fs' , refl , ok) = 
  inj₂ ((R , (` X , tt) , ax) ∷ (t , C' , f') ∷ fs' , refl , {!   !})
check-focus {.- , snd} (.(I , tt) , f2li Ir) ((C' , .(f2li (untagF f'))) ∷ .(mapList (λ x → proj₁ (proj₂ x) , f2li (untagF (proj₂ (proj₂ x)))) fs')) | inj₂ ((t , C' , f') ∷ fs' , refl , ok) = 
  inj₂ ((R , (I , tt) , Ir) ∷ (t , C' , f') ∷ fs' , refl , {!   !})
check-focus { - , snd} (C , f2li (pass f)) ((C' , .(f2li (untagF f'))) ∷ .(mapList (λ x → proj₁ (proj₂ x) , f2li (untagF (proj₂ (proj₂ x)))) fs')) | inj₂ ((t , .C' , f') ∷ fs' , refl , snd₂) = inj₂ ((P , C , passT f) ∷ (t , C' , f') ∷ fs' , refl , {!   !})
check-focus {S , q} (.(A' ⊗ B' , tt) , f2li (⊗r l {S = .(irr (S , q)) , p} {A = A'} {B'} ok₁ eq f g)) ((C' , .(f2li (untagF f'))) ∷ .(mapList (λ x → proj₁ (proj₂ x) , f2li (untagF (proj₂ (proj₂ x)))) fs')) | inj₂ ((t , C' , f') ∷ fs' , refl , ok) 
  rewrite irr-eq S p | isIrr-unique S p q = inj₂ ((R , (A' ⊗ B' , tt) , ⊗rT l ok₁ eq f g) ∷ (t , C' , f') ∷ fs' , refl , {!   !})
check-focus {.(just (_ ∧ _)) , snd} (C , f2li (∧l₁ f)) ((C' , .(f2li (untagF f'))) ∷ .(mapList (λ x → proj₁ (proj₂ x) , f2li (untagF (proj₂ (proj₂ x)))) fs')) | inj₂ ((t , C' , f') ∷ fs' , refl , ok) = 
  inj₂ ((C₁ , C , ∧l₁T f) ∷ (t , C' , f') ∷ fs' , refl , isC₁ (t ∷ mapList (λ r → proj₁ r) fs') (isOK2isOKC₁ f' ok))-- isOK2isOKC₁ f' ok
check-focus {.(just (_ ∧ _)) , snd} (C , f2li (∧l₂ f)) ((C' , .(f2li (untagF f'))) ∷ .(mapList (λ x → proj₁ (proj₂ x) , f2li (untagF (proj₂ (proj₂ x)))) fs')) | inj₂ ((t , C' , f') ∷ fs' , refl , ok) = 
  inj₂ ((C₂ , C , ∧l₂T f) ∷ (t , C' , f') ∷ fs' , refl , {!   !}) -- isOK2isOKC₂ f' ok
check-focus {S , q} (.(A' ∨ B' , tt) , f2li (∨r₁ l {S = .(irr (S , q)) , p} {A = A'} {B'} ok₁ f)) ((C' , .(f2li (untagF f'))) ∷ .(mapList (λ x → proj₁ (proj₂ x) , f2li (untagF (proj₂ (proj₂ x)))) fs')) | inj₂ ((t , C' , f') ∷ fs' , refl , ok) 
  rewrite irr-eq S p | isIrr-unique S p q = inj₂ ((R , (A' ∨ B' , tt) , ∨r₁T l ok₁ f) ∷ (t , C' , f') ∷ fs' , refl , {!   !})
check-focus {S , q} (.(A' ∨ B' , tt) , f2li (∨r₂ l {S = .(irr (S , q)) , p} {A = A'} {B'} ok₁ f)) ((C' , .(f2li (untagF f'))) ∷ .(mapList (λ x → proj₁ (proj₂ x) , f2li (untagF (proj₂ (proj₂ x)))) fs')) | inj₂ ((t , C' , f') ∷ fs' , refl , ok) 
  rewrite irr-eq S p | isIrr-unique S p q = inj₂ ((R , (A' ∨ B' , tt) , ∨r₂T l ok₁ f) ∷ (t , C' , f') ∷ fs' , refl , {!   !})
